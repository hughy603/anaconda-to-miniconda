...
from .utils import PathLike, logger, setup_logging
...
def main(args: Sequence[str] | None = None) -> int:
    ...
    if parsed_args.batch:
        ...
        elif Path(parsed_args.source_env).is_dir() and is_conda_environment(parsed_args.source_env):
            # If the source_env is a path and is a conda environment
            env_path = parsed_args.source_env
            parsed_args.source_env = Path(parsed_args.source_env).name
            if parsed_args.verbose:
                logger.debug(f"Using environment at path: {env_path}")
        elif parsed_args.search_path:
            logger.error(
                f"Environment '{parsed_args.source_env}' not found in registered environments or search paths",
            )
            return 1
